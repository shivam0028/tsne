# -*- coding: utf-8 -*-
"""
Created on Sat Aug  3 21:44:50 2019
...
def scatter_simple(x):

    # Create a scatter plot
    plt.figure(figsize=(8, 8))
    ax = plt.subplot(aspect='equal')
    ax.scatter(x[:, 0], x[:, 1], lw=0, s=40, c='blue')
...
def p_matrix(X=np.array([]), sigma=5.0):
    ...
    P = np.exp(D)
    P[range(n), range(n)] = 0
    # Divide each element by sum of matrix
    P = P / sum(P)
...
def symmetric_sne(X=np.array([]), no_dims=2, initial_dims=50, max_iter=1000,
    ...
    for iter in range(max_iter):
        ...
        if iter > 100 and C[iter-100] - C[iter] < 0.01:
...
def run(dataset='mnist'):
    ...
    if dataset == 'swissroll':
        print("Running example on 2,500 Swiss Roll Points.")
        X, labels = make_swiss_roll(n_samples=500)
        X = normalize(X)
        n_classes = 0

    if dataset == 'diagonal':
        print("Running example on 100 Diagonal Points.")
        X = np.full((1, 100), 1)
        for i in range(2, 101):
            thisR = np.full((1, 100), i)
            X = np.concatenate((X, thisR))
    ...
    for sigma in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        Y, C = symmetric_sne(X, no_dims=2, initial_dims=50,
                             max_iter=5000, sigma=sigma)
        if dataset == 'mnist':
            scatter(Y, labels, n_classes=n_classes)
        else:
            scatter_simple(Y)
        plt.savefig('symmetric_sne_'+dataset+'_sigma_'+str(sigma)+'.eps',
                    format='eps', dpi=1000)
        plt.clf()
        plt.plot(C)
        plt.savefig('symmetric_sne_'+dataset+'_sigma_cost_'+str(sigma)+'.eps',
                    format='eps', dpi=1000)
        plt.clf()
    ...
    for perplexity in [5.0, 10.0, 15.0, 20.0, 30.0, 50.0, 100.0]:
        ...
        Y, C = symmetric_sne(X, no_dims=2, initial_dims=50, max_iter=5000,
                             perplexity=perplexity, use_perplexity=True)
        if dataset == 'mnist':
            scatter(Y, labels, n_classes=n_classes)
        else:
            scatter_simple(Y)
        plt.savefig('symmetric_sne_'+dataset+'_perplexity_'+str(perplexity)+'.eps',
                    format='eps', dpi=1000)
        plt.clf()
        plt.plot(C)
        plt.savefig('symmetric_sne_'+dataset+'_perplexity_cost_'+str(perplexity)+'.eps',
                    format='eps', dpi=1000)
        plt.clf()
